<script lang="ts">
  ...
  import type { AgentCommandRequest, AgentCommandState } from '../../../src/shared/contracts.js';
              ...
              {:else}
                {#each state.models as model}
                  <button
                    ...
                    type="button"
                    disabled={busy || !model.available || model.isCurrent}
                  ...
                  >
                    ...
                    <span class="command-meta">{model.isCurrent ? 'Current' : model.available ? (model.usingSubscription ? 'OAuth' : 'Ready') : 'Unavailable'}</span>
                  </button>
                {/each}
              {/if}
            </div>
